...
- 项目中文注释和 README 多处是乱码，应该是编码不一致导致的，不影响架构梳理，但会影响后续维护体验。
- `seckill.lua` 当前末尾执行了 `xadd`，但没有显式 `return 0`；而 Java 侧按 `result == 0` 判断成功。这个点后续最好检查一下，否则秒杀入口可能会把 Lua 返回 `nil` 当成失败。
